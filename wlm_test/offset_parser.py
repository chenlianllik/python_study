...
def wlm_offset_parser(csv_file_name):
	...
	with open(csv_file_name) as csv_file:
		...
		for row in reader:
			if 'WMITLV_TAG_ARRAY_STRUC' in row[0]:
				struct_array_num = int(row[1][(len(row[1])-4):], 16)
			elif 'WMITLV_TAG_STRUC' in row[0]:
				struct_size = int(row[1][(len(row[1])-4):], 16)
				struct_array_num = struct_array_num/(struct_size+4)
